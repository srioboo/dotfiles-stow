...
function getMaxRunTimeMs(): number {
  const configuredMinutes = Number.parseInt(process.env.OPENCODE_DELEGATION_MAX_MINUTES ?? "", 10)
...
const MAX_RUN_TIME_MS = getMaxRunTimeMs()
...
async function buildDelegationPrompt(
  ...
  const sections = [
    `You are executing a delegated task from agent "${input.parentAgent}" in project "${path.basename(projectDirectory)}".`,
  ]

  const projectInstructions = await readOptionalText(path.join(projectDirectory, "AGENTS.md"))
  if (projectInstructions) {
    sections.push(
      `Project instructions:\n\n${truncateText(projectInstructions, 2200)}`,
    )
  }

  const skillRegistry = await readOptionalText(path.join(projectDirectory, ".atl", "skill-registry.md"))
  if (skillRegistry) {
    const compactRules = extractMarkdownSection(skillRegistry, "Compact Rules")
    if (compactRules) {
      sections.push(
        `## Project Standards (auto-resolved)\n\n${truncateText(compactRules, 2200)}`,
      )
    }

    const projectConventions = extractMarkdownSection(skillRegistry, "Project Conventions")
    if (projectConventions) {
      sections.push(
        `## Project Conventions\n\n${truncateText(projectConventions, 1200)}`,
      )
    }
  } else {
    sections.push(
      "Project standards:\n\nNo skill registry found at `.atl/skill-registry.md`. Proceed without auto-resolved standards and report `Skill Resolution: none` if asked.",
    )
  }

  sections.push(`Delegation guardrails:
- Follow your own role-specific instructions and tools.
- Treat the project instructions above as binding when relevant.
- Treat any injected Project Standards or Project Conventions as binding when relevant.
- Keep the scope narrow and preserve unrelated user changes.
- Return compact evidence, validation, and residual risk.`)

  sections.push(`Delegated task:\n\n${input.prompt}`)
...
class DelegationManager {
  ...
  constructor(client: OpencodeClient, baseDir: string, projectDirectory: string, log: Logger) {
    this.client = client
  ...
  async delegate(input: DelegateInput): Promise<Delegation> {
    ...
    const delegatedPrompt = await buildDelegationPrompt(this.projectDirectory, input)
    ...
    this.client.session
      .prompt({
        ...
        body: {
          ...
          parts: [{ type: "text", text: delegatedPrompt }],
          tools: {
            task: false,
            delegate: false,
            todowrite: false,
...
Use delegation_read(id) to retrieve the full result.`
  ...
  /**
   * Read a delegation's output by ID. Never blocks the parent session.
   */
  async readOutput(sessionID: string, id: string): Promise<string> {
    ...
    // Important: do not block here. A blocking read caused parent sessions to stall when
    // the orchestrator asked too early for a background result.
    const delegation = this.delegations.get(id)
    if (delegation) {
      if (delegation.status === "running") {
        await this.debugLog(`readOutput: delegation ${delegation.id} still running; returning non-blocking status`)
        return `Delegation "${delegation.id}" is still running. This read is non-blocking: continue with other useful work, wait for the task notification, or use native task next time if you need a blocking result.`
      }
...
${delegation.result}`
      ...
      const title = delegation.title || delegation.id
      return `Delegation "${title}" ended with status: ${delegation.status}. ${delegation.error || ""}`.trim()
    }
...
Use \`delegation_read(id)\` to retrieve the full result. Results are persisted to disk and survive compaction.`,
    args: {
      prompt: tool.schema
        .string()
        .describe("The full detailed prompt for the agent. Use the language that best matches the project and the user."),
      agent: tool.schema
        .string()
        .describe(
          'Agent to delegate to. Prefer project subagents such as "oo-analista", "oo-implementador", "oo-qa", "oo-navegador", "oo-abogado-diablo", "oo-estado" or the expensive final gate "oo-claudevil" only when explicitly requested or after a substantially large task is complete.',
        ),
    },
    async execute(args: DelegateArgs, toolCtx: ToolContext): Promise<string> {
      ...
      try {
        ...
        response += `\nYou WILL be notified when ${totalActive > 1 ? "ALL complete" : "complete"}. delegation_read is safe for a quick non-blocking status/result check; do not wait idle.`
...
- Native \`task\` may also exist for synchronous sub-agent calls, depending on the active agent permissions

## When to Use delegate vs task

| Tool | Behavior | Use When |
|------|----------|----------|
| \`task\` | Synchronous, blocks until complete | You need the result before deciding, or you are running from one-shot CLI |
| \`delegate\` | Async, background, persisted to disk | The session will stay alive and you can do other useful work |

Prefer project subagents such as \`oo-analista\`, \`oo-implementador\`, \`oo-qa\`, \`oo-navegador\`, \`oo-abogado-diablo\`, and \`oo-estado\`.
Use \`oo-claudevil\` only when explicitly requested or as the final expensive Claude Code review after a substantially large task is complete and locally validated.
Results survive context compaction when the OpenCode process remains alive.
...
**Do not poll \`delegation_list\` to check completion.**
You WILL be notified via \`<task-notification>\`. If you need to inspect a known delegation ID, \`delegation_read(id)\` is non-blocking and returns either the result or a running status.

**NEVER wait idle.** Always have productive work while delegations run.

If you cannot continue without the result, use \`task\` instead of \`delegate\`.
...
function formatDelegationContext(
  ...
  if (running.length > 0) {
    ...
    sections.push("> Do not poll `delegation_list`; use `delegation_read(id)` only for a quick non-blocking status/result check, then continue productive work.")
    sections.push("")
  }
...
export const BackgroundAgents: Plugin = async (ctx) => {
  ...
  const manager = new DelegationManager(client as OpencodeClient, baseDir, directory, log)
