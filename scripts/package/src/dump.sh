...
if platform::is_macos; then
	HOMEBREW_DUMP_FILE_PATH="$DOTFILES/os/mac/brew/Brewfile"
elif platform::is_linux; then
	HOMEBREW_DUMP_FILE_PATH="$DOTFILES/os/linux/brew/Brewfile"
fi

APT_DUMP_FILE_PATH="$DOTFILES/os/linux/apt/packages.txt"
CODE_DUMP_FILE_PATH="$DOTFILES/editors/code/extensions.txt"
NPM_DUMP_FILE_PATH="$DOTFILES/langs/js/global_modules.txt"
PACMAN_DUMP_FILE_PATH="$DOTFILES/os/linux/pacman/packages.txt"
PYTHON_DUMP_FILE_PATH="$DOTFILES/langs/python/requirements.txt"
SNAP_DUMP_FILE_PATH="$DOTFILES/os/linux/snap/packages.txt"
VOLTA_DUMP_FILE_PATH="$DOTFILES/langs/js/volta_dependencies.txt"
WINGET_DUMP_FILE_PATH="$DOTFILES/os/windows/winget.output"

package::brew_dump() {
	if platform::is_macos; then
		mkdir -p "$DOTFILES/os/mac/brew"
	else
		mkdir -p "$DOTFILES/os/linux/brew"
	fi
...
package::snap_dump() {
	mkdir -p "$DOTFILES/os/linux/snap"
...
package::pacman_dump() {
	mkdir -p "$DOTFILES/os/linux/pacman"
