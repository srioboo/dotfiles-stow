...
dot::list_contexts() {
	dotly_contexts=$(ls "$DOTFILES_PATH/scripts")
...
dot::list_context_scripts() {
	...
	dotly_scripts=$(ls -p "$DOTFILES_PATH/scripts/$context" 2>/dev/null | grep -v '/')
...
dot::list_scripts_path() {
	dotly_contexts=$(find "$DOTFILES_PATH/scripts" -maxdepth 2 -perm /+111 -type f | grep -v "$DOTFILES_PATH/scripts/core")
